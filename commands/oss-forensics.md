...
# /lyrie-intel - OSS GitHub Forensic Investigation
...
1. **Read the orchestration skill:**
   `.claude/skills/lyrie-intel/orchestration/SKILL.md`

2. **Follow the workflow** defined in that skill exactly

3. **The user's investigation request is:**
   {rest of command arguments after /lyrie-intel}
...
All results will be saved to: `.out/lyrie-intel-{timestamp}/`
...
- **GOOGLE_APPLICATION_CREDENTIALS**: BigQuery credentials for GH Archive queries
  - See `.claude/skills/lyrie-intel/github-archive/SKILL.md` for setup
...
```
/lyrie-intel "Investigate lkmanka58's activity on aws/aws-toolkit-vscode"

/lyrie-intel "Validate claims in this vendor report: https://example.com/report"

/lyrie-intel "What happened with the stability tag on aws/aws-toolkit-vscode on July 13, 2025?"

/lyrie-intel "Investigate the July 13 incident on aws/aws-toolkit-vscode" --max-followups 5
```
