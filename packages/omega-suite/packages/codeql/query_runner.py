...
class QueryRunner:
    ...
    def run_suite(
        ...
        cmd = [
            ...
            "--format=sarif-latest",
            f"--output={sarif_path}",
            f"--threads={LyrieConfig.CODEQL_THREADS}",
            f"--ram={LyrieConfig.CODEQL_RAM_MB}",
        ...
        logger.info(f"Timeout: {LyrieConfig.CODEQL_ANALYZE_TIMEOUT}s")
        ...
        try:
            result = subprocess.run(
                ...
                capture_output=True,
                text=True,
                timeout=LyrieConfig.CODEQL_ANALYZE_TIMEOUT,
            )
            ...
            error = f"Analysis timed out after {LyrieConfig.CODEQL_ANALYZE_TIMEOUT}s"
            errors.append(error)
