...
def _find_script(tmp_path):
    """Find the generated .lyrie_build_*.py script in tmp_path."""
    scripts = list(tmp_path.glob(".lyrie_build_*.py"))
...
class TestSynthesiseCpp:
    ...
    def test_build_dir_created_for_codeql(self, tmp_path):
        ...
        build_dirs = [p for p in tmp_path.glob(".lyrie_build_*") if p.is_dir()]
        scripts = [p for p in tmp_path.glob(".lyrie_build_*.py")]
