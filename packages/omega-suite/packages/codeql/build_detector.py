...
class BuildDetector:
    ...
    def synthesise_build_command(self, language: str) -> Optional[BuildSystem]:
        ...
        import tempfile
        build_dir = Path(tempfile.mkdtemp(prefix=".lyrie_build_", dir=self.repo_path))
        fd, script_name = tempfile.mkstemp(
            prefix=".lyrie_build_", suffix=".py", dir=self.repo_path,
        )
        os.close(fd)
...
"""Synthesised by Lyrie OMEGA for CodeQL database creation.
