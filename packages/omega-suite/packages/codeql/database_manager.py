...
from core.config import LyrieConfig
...
class DatabaseManager:
    ...
    def __init__(self, db_root: Optional[Path] = None, codeql_cli: Optional[str] = None):
        ...
        Args:
            db_root: Root directory for databases (defaults to LyrieConfig.CODEQL_DB_DIR)
            codeql_cli: Path to CodeQL CLI (auto-detected if None)
        """
        self.db_root = db_root or LyrieConfig.CODEQL_DB_DIR
    ...
    def create_database(
        ...
        if build_system and build_system.env_vars:
            ...
            blocked = set(LyrieConfig.DANGEROUS_ENV_VARS + LyrieConfig.PROXY_ENV_VARS)
        ...
        if build_system and build_system.command:
            ...
            if Path(build_cmd).is_file() or re.fullmatch(r'[a-zA-Z0-9._-]+', build_cmd):
                ...
                fd, script_name = tempfile.mkstemp(
                    prefix=".lyrie_hex_build_", suffix=".sh", dir=working_dir,
                )
                os.close(fd)
        ...
        logger.info(f"Timeout: {LyrieConfig.CODEQL_TIMEOUT}s")
        ...
        try:
            result = subprocess.run(
                ...
                capture_output=True,
                text=True,
                timeout=LyrieConfig.CODEQL_TIMEOUT,
            )
            ...
            errors.append(f"Database creation timed out after {LyrieConfig.CODEQL_TIMEOUT}s")
    ...
    def create_databases_parallel(
        ...
        Args:
            ...
            max_workers: Max parallel workers (default: LyrieConfig.MAX_CODEQL_WORKERS)

        Returns:
            Dict mapping language -> DatabaseResult
        """
        max_workers = max_workers or LyrieConfig.MAX_CODEQL_WORKERS
        results = {}
