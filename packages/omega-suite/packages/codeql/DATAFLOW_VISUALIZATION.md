...
The CodeQL integration now includes automatic **dataflow visualization** for all findings that contain data flow information. When CodeQL detects a taint-tracking or dataflow vulnerability, Lyrie OMEGA automatically generates visual representations in multiple formats.
...
Lyrie OMEGA generates **4 different visualization formats** for each dataflow finding:
...
python3 lyrie_omega.py \
  --repo /path/to/code \
  --codeql \
  --languages java
```

**Note**: The integrated workflow (lyrie_omega.py) currently generates visualizations only for the standalone CodeQL analysis. Full integration with the unified autonomous analysis is planned for a future release.
...
grep "visualizations" lyrie_hex.log
```

**Check 3**: Check for errors
```bash
# Look for visualization errors in logs
grep -i "visualization.*error" lyrie_hex.log
```
...
- [ ] **Integration improvements**
  - Full integration with lyrie_omega.py unified workflow
